...
const getPageSize = function (start, end = new Date().toLocaleDateString()) {
    let result = 20;
...
const prices = async function (code, start, end, options) {
    const url = new URL('https://api.fund.eastmoney.com/f10/lsjz');

    url.searchParams.set('fundCode', code);
    url.searchParams.set('pageIndex', 1);
    url.searchParams.set('pageSize', getPageSize(start, end));
